...
_start:
    ...
    mov rdi, valInput
    call _strLenFromInput
    mov rcx, rax                    ;передаём длину строки
;    mov qword [valInput+rcx+1], 0  ;добавляем окончание строки
;    lea rsi, [valInput]
    call _ASCIItoNum
    xor r8, r8
...
_strLen:
  ...
  dec   rax                 ; убираем последнее значение т.к. это 0 (так делать плохо!)
